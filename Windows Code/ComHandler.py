...
class ArduinoCommunication:
    def __init__(self, settings, changed_color, log):
        ...
        if self.port_found:
            for num, port in enumerate(self.port):
                try:
                    ...
                    self.l.logger('connected to {}'.format(port), debugging_state)
                    break
                except serial.serialutil.SerialException:
                    self.l.logger('tried to connect to {} but attempted failed'.format(port), debugging_state)
                    print(port)
    ...
    def get_arduino_port(self):
        ...
        if  self.port_found:
            self.l.logger('Er zijn {} arduinos gevonden. Er wordt verbinding gemaakt met de eerste arduino of de eene die '
